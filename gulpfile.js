...
gulp.task("Package-Publish",
    function(callback) {
        ...
        runSequence(
            ...
            "Publish-Unicorn",
            "Publish-Azure-Specific",
            callback);
    });

gulp.task("Publish-Unicorn", function ()
{
    var allowedPatterns = [
    ...
    gulp.src(allowedPatterns).pipe(gulp.dest(config.websiteRoot + "/Serialization"));
});

gulp.task("Publish-Azure-Specific", function () {
    // the azure folder contains deployment specific files. This is not optimal and should be done otherwise.
 
    var allowedPatterns = [
        "./Azure/**/*.*"
     ];

    gulp.src(allowedPatterns).pipe(gulp.dest(config.websiteRoot));
});


/* Remove unwanted files */
gulp.task("Package-Remove-Package-Files", function () {
...
/* Generate a big zip to deploy */
gulp.task("Generate-Zip", function (callback) {
    runSequence(
        "Package-Clean",
        "Package-Publish",
        "Package-Remove-Package-Files",
        callback);
});
